    ...
    return (
        <div className="flex flex-col gap-2">
            <div className="w-full text-sm text-muted-foreground">
                {category === 'Currency' ? (
                    <>
                        Your shares: Yes - {Math.floor(parseInt(toEther(sharesBalance?.optionAShares)))}, No - {Math.floor(parseInt(toEther(sharesBalance?.optionBShares)))}
                    </>
                ) : (
                    <>
            ...
            {(winnings.A > 0 || winnings.B > 0) && (
                <div className="flex flex-col gap-1">
                    ...
                    <div className="flex gap-2">
                        <Badge variant="secondary">{category === 'Currency' ? 'Yes' : market.optionA}: {displayWinningsA} shares</Badge>
                        <Badge variant="secondary">{category === 'Currency' ? 'No' : market.optionB}: {displayWinningsB} shares</Badge>
                    </div>
                </div>
            )}
        </div>
